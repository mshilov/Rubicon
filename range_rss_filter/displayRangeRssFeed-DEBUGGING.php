...
//if blank it uses todays date time to initiate variables and results in returning timestamp
 public function cnctStartDate($year1,$month1,$day1,$time,$ampm1)
 {
 	...
 	// now we check if time was empty string and use NOW timestamp for start time of the event 
 	if ($time =='')
 	{ 	
 	...
 	// checking for empty time and extracting hour and minutes from NOW or if $time was set then getting it from arrayStartime that was build earlier
 ...
 public function bldRSSfeed($path)
	...
	foreach ($resultDOM as $node) {
			...
			//}//exclude condt.
			 ...
			 //parse through array and extract date range for each event.
...
$this->cnctEndDate($items[$item_date]["year2"],$items[$item_date]["month2"],$items[$item_date]["day2"],$items[$item_date]["endtime"],$items[$item_date]["ampm2"],$items[$item_date]["year1"],$items[$item_date]["month1"],$items[$item_date]["day1"]),$items[$item_date]["title"],$items[$item_date]["link"],$items[$item_date]["description"],$items[$item_date]["pubdate"]);
		...
		// while still being in foreach loop send each array to the range comparison function it will flter out the current items to display and return it as $items.
...
//$arrDateRange is an array, consisting from Start date and End Date Unit timestamps of the event  and $now is accurrent timestamps
	...
	// $title = $arrDateRange[2];
	 //$link = $arrDateRange[3];
	 //$description = $arrDateRange[4];
	// $pubdate = $arrDateRange[5];
	
	//for DEBUGGING test cases recalculates Unix timestamp into array of day, month, year, min and hours..	
	//$st =  getDate($startDate);
	//$ed =  getDate($endDate);
...
if ($startDate <= $now && $endDate >= $now )
	...
	/* echo "<br/><font style='color:red;'>".$title ."</font><br/>". "Start Date: " .  $st['year'] . "-" . $st['month'] . "-" . $st['mday'] ." " . $st['hours'] . ":". $st['minutes'] .  "<br/> End Date: " .  $ed['year'] . "-" . $ed['month'] . "-" . $ed['mday'] ." " . $ed['hours'] . ":". $ed['minutes']."<br/> now is ".$now ." <br/>_____________________________" . "<br/>"; */
	...
	// $endDate < $now which makes event to be in the past
		...
		if ($startDate > $now)
			{
			// DEBUGGIN event is in the future treat it as a future event
		/*	$result = "<br/>Event on " . $startDate . " is in the Future.<br/>"."and now is ". $now."<br/>____________________________________ " . "<br/><br/>";
			echo"$result";
	  echo "Start Date: " .  $st['year'] . "-" . $st['month'] . "-" . $st['mday'] ." " . $st['hours'] . ":". $st['minutes'] .  "<br/> End Date: " .  $ed['year'] . "-" . $ed['month'] . "-" . $ed['mday'] ." " . $ed['hours'] . ":". $ed['minutes'] ."<br/>"."_________________________________". "<br/>"; */
			return NULL;
			} 
		...
		if ($endDate < $now )
			...
			// DEBUGGING do nothing , it's a past event
		/*	$result = "<br/>Event on " . $endDate . " has successfully passed.<br/>"."and now is ". $now ."<br/>_________________________________". "<br/><br/>";;
		echo"$result";
